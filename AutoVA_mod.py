...
class IVProcedure(Procedure):
    ...
    def startup(self):
        ...
        # self.ahe_source.disable_source()
    ...
    def execute(self):
        self.ahe_keithley.disable_source()
        self.kepcosource.write("OUTPUT OFF")
                        ...
                        if temp == -1:
                            for i, voltage in enumerate(minus_voltages_up):
                                ...
                                if i == len(minus_voltages_up) - 1:
                                    ...
                                    #     # start the AHE measurements in parallel
                                    #     executor.map(self.measure_ahe_data, steps)

                                    for i, fieldcurrent in enumerate(steps):
                                        thread = Thread(target=self.measure_ahe_data, args=(steps[i],))
                                        thread.start()
                                    #     self.kepcosource.write("CURR %f"%(fieldcurrent))
                                    #     hallvoltage = self.ahe_keithley.voltage
                                    #     real_fieldcurrent = float(self.kepcosource.query("MEAS:CURR?"))
    ...
    def measure_ahe_data(self, fieldcurrent):
        CurrentData = []
        HallVData = []
        self.kepcosource.write("CURR %f" % (fieldcurrent))
        hallvoltage = self.ahe_keithley.voltage
        ...
        return real_fieldcurrent, hallvoltage
