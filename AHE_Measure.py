...
class IVProcedure(Procedure):
    ...
    def startup(self):
        ...
        self.ahe_meter.voltage_nplc = 1 # Integration constant to Medium
    ...
    def AHE_loop(self):
        ...
        for i, fieldcurrent in enumerate(steps):
            ...
            self.HallVData.append(self.ahe_meter.voltage)
            data = {
                'FieldCurrent (A)': real_fieldcurrent,
                'Hall Voltage (V)': self.ahe_meter.voltage,
            }
    ...
    def execute(self):
        self.Bias_Current()
        thread = Thread(target=self.AHE_loop)
        thread.start()
