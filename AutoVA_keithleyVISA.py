...
class IVProcedure(Procedure):
    ...
    # devices = IntegerParameter('Devices', units='No.', default=1)
    ...
    def startup(self):
        ...
        self.meter = Keithley2400("GPIB1::24::INSTR")
        self.meter.measure_current()
        self.meter.current_range = self.current_range * 1e-6
        self.meter.current_nplc = 0.01  # Integration constant to Medium

        self.source = Keithley2400("GPIB1::24::INSTR")
        self.source.apply_voltage()
        #self.source.source_voltage_range = self.max_voltage * 1e-3  # V
        self.source.source_voltage_range = 20   #source voltage
        self.source.compliance_current = self.current_range * 1e-6 # A
        self.source.enable_source()
        sleep(2)
        '''
        #set up the KEPCO and Keithley for the AHE measurement
        #step1: get the the address of the kepcosource
        '''
        ...
        self.ahe_keithley = Keithley2450("GPIB0::18::INSTR")
        self.ahe_keithley.write("*RST")
        self.ahe_keithley.write("*CLS")
        self.ahe_keithley.wires = 4
        self.ahe_keithley.use_front_terminals()
        self.ahe_keithley.apply_current()
        self.ahe_keithley.compliance_voltage = 5                      # set the compliance voltage to 5V
        self.ahe_keithley.write(":SENS:FUNC 'VOLT' ")
        self.ahe_keithley.write(":OUTP ON")
        self.ahe_keithley.enable_source()                # Enables the source output
        self.ahe_keithley.measure_voltage()              # Sets up to measure voltage 
        # self.ahe_source = Keithley2450("GPIB0::18::INSTR")
        # self.ahe_source.wires = 4
        # self.ahe_source.use_front_terminals()
        # self.ahe_source.apply_current()
        # self.ahe_source.compliance_voltage = 5                      # set the compliance voltage to 5V
        # self.ahe_source.source_current_range = 10
        # self.source_current = 0
        

        # self.ahe_meter = Keithley2450("GPIB0::18::INSTR")
        # self.ahe_source.enable_source()
        # self.ahe_meter.measure_voltage()
    ...
    def execute(self):
        ...
        # self.duration_ef_1 = abs(self.max_voltage - self.min_voltage) / self.voltage_step * self.delay * 1e-3 * 2
        # self.duration_ef_2 = self.voltage_dwell_time
        # self.duration_ahe = abs(self.AHE_field) / self.AHE_field_step * self.AHE_waiting_time * 5
        # self.duration = self.duration_ef_1 + self.duration_ef_2 + self.duration_ahe
        ...
        if self.max_voltage > 0:
                                ...
                                if i == len(voltages_down) - 1:
                                    ...
                                    for i, fieldcurrent in enumerate(steps):
                                        ...
                                        if i == len(steps) - 1:
                                            ...
                                            self.ahe_keithley.source_current = 0
                                            self.kepcosource.write("CURR 0.0")
                                            self.ahe_keithley.beep(frequency = 200, duration = 2)
                                        ...
                                        if self.should_stop():
                                            log.warning("Catch stop command in procedure")
                                            break
                                    # pfolder_dir = MainWindow().queue().self.directory
                                    ...
                                    #     parent_path = os.path.abspath(os.path.join(pfolder_dir, os.pardir)) + '\\AHE' + folder_name
            ...
            # voltages = np.concatenate((minus_voltages_up, minus_voltages_down))
            # steps = len(voltages) + int(self.voltage_dwell_time)
                            ...
                            for i, voltage in enumerate(minus_voltages_up):
                                ...
                                if i == len(minus_voltages_up) - 1:
                                    ...
                                    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                                        futures = [executor.submit(func) for func in [self.apply_ahe_current, self.read_EM_field, self.measure_hall_voltage, self.get_data]]
                                        for i, fieldcurrent in enumerate(steps):
                                            future_EM_field = executor.submit(self.apply_EM_filed, fieldcurrent)
                                            futures.append(future_EM_field)
                                            concurrent.futures.wait(futures)
                                            data = {
                                                'FieldCurrent (A)': futures[1].result(),
                                                'Hall Voltage (V)': futures[2].result(),
                                            }
                                            self.emit('results', data)
                                            self.emit('progress', 100. * (self.progress_ef_1 + self.voltage_dwell_time + self.progress_ef_3 + i) / self.al_progress)
                                            CurrentData.append(futures[3].result()[0])
                                            HallVData.append(futures[3].result()[1])
                                            np.savetxt(parent_path+'\\CurrentvsHallV.txt',np.c_[CurrentData,HallVData],fmt='%s') 
    ...
    def apply_EM_filed(self, fieldcurrent):
        self.kepcosource.write("CURR %f"%(fieldcurrent))

    def read_EM_field(self):
        real_fieldcurrent = float(self.kepcosource.query("MEAS:CURR?"))
        return real_fieldcurrent
    
    def get_data(self):
        data = {
            'FieldCurrent (A)': self.read_EM_field(),
            'Hall Voltage (V)': self.measure_hall_voltage(),
        }
        return data
...
class MainWindow(ManagedDockWindow):
    def __init__(self):
        ...
        self.setWindowTitle('IV Measurement & AHE Measurement Automation')
    ...
    def queue(self, procedure=None):
        ...
        #     experiment = self.new_experiment(results)
        #     self.manager.queue(experiment)
        # elif IVProcedure().devices == 2:
        #     procedure = AHE_procedure()
        #     procedure = self.make_procedure()
        #     results = Results(procedure, filename)
        #     experiment = self.new_experiment(results)
        #     self.manager.queue(experiment)
        # elif IVProcedure().devices == 3:
        #     procedure = IVProcedure()
        #     results = Results(procedure, filename)
        #     experiment = self.new_experiment(results)
        #     self.manager.queue(experiment)          
        #     if IVProcedure().shutdown():
        #         procedure = AHE_procedure()
        #         results = Results(procedure, filename)
        #         experiment = self.new_experiment(results)
        #         self.manager.queue(experiment)
