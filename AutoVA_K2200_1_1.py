...
AutoVA_1.1.1, Features:
add the email information to send the email to the user when the measurement is finished
send the email if the voltage does not reach the max voltage
...
from pymeasure.experiment import (
    Procedure, FloatParameter, IntegerParameter, Parameter, unique_filename, Results, Worker
)
...
class IVProcedure(Procedure):
    ...
    send_email = Parameter('Email Address to get notifications', default='kammichan90@gmail.com')
    send_email_password = Parameter('Password for the email address', default='kammichan90')
    CurrentData = []
    HallVData = []
    ...
    def notification_too_high_voltage(self):
        sender_email = self.send_email
        password = self.send_email_password
        receiver_email = self.send_email

        message = MIMEMultipart("mixed")
        message["Subject"] = "The voltage does not reach the max voltage"
        ...
        <body>
            <p>Hi,<br>
            How are you?<br>
            The target voltage does not reach the max voltage. <br>
            Please check the setup.<br>
            </p>
        </body>
        </html>
        """
        ...
        try:
            ...
            log.info("The password is wrong!")
            pass


    def notification_finished(self):
        sender_email = self.send_email
        password = self.send_email_password
        receiver_email = self.send_email

        message = MIMEMultipart("mixed")
        message["Subject"] = "The measurement is finished"
        message["From"] = sender_email
        message["To"] = receiver_email
        ...
        <body>
            <p>Hi,<br>
            How are you?<br>
            The measurement is finished. <br>
            </p>
        </body>
        </html>
        """
            ...
            for i, voltage in enumerate(voltages_up):
                ...
                if voltage == (self.max_voltage * 1e-3) or voltage > (self.max_voltage - self.voltage_step) * 1e-3:
                    ...
                    while temp > -1:
                        ...
                        if self.mes_voltage < self.max_voltage * 1e-3:
                            self.notification_too_high_voltage()
                        if self.should_stop():
                            log.warning("Catch stop command in procedure")
                            self.IV_source.shutdown() 
            ...
            for i, voltage in enumerate(minus_voltages_down):
                ...
                if voltage == (self.min_voltage * 1e-3) or voltage < (self.min_voltage + self.voltage_step) * 1e-3:
                    ...
                    while temp > -1:
                        ...
                        self.emit('results', data)
                        self.emit('progress', 100. * (self.progress_ef_1 + temp_rest) / self.al_progress)
                        if self.mes_voltage > self.min_voltage * 1e-3:
                            self.notification_too_high_voltage()
                        if self.should_stop():
                            log.warning("Catch stop command in procedure")
                            self.IV_source.shutdown() 
    ...
    def __init__(self):
        super(MainWindow, self).__init__(
            ...
            inputs=[
                ...
                'delay', 'current_range', 'interval', 'AHE_field', 'AHE_field_step', 'AHE_waiting_time', 'AHE_bias_current', 'send_email', 'send_email_password'
            ],
            displays=[
                'max_voltage', 'voltage_dwell_time', 'min_voltage', 'voltage_step', 
                'delay', 'current_range', 'interval', 'AHE_field', 'AHE_field_step', 'AHE_waiting_time', 'AHE_bias_current', 'send_email', 'send_email_password'
